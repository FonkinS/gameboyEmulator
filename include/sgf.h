...
SGFtexture SGFLoadTexture(char* texturePath, int isAntiAliased) {
    ...
    if (texturePath == NULL) return texture;
...
SGFsprite SGFCreateBitmapSprite(SGFwindow win, int x, int y, int w, int h, int tex_w, int tex_h, unsigned char* data, int alpha) {
    SGFsprite s;
    s.object = SGFCreateSquare(win, x, y, w, h);
    s.texture = SGFLoadTexture(NULL, 0);
    SGFSetBitmapTexture(s, tex_w, tex_h, data, alpha);
    return s;
}
