...
uint8_t CH1_wave_index = 0;
uint8_t CH2_wave_index = 0;

uint8_t appending_timer = 0;

uint8_t current_clip[24000];
int clip_index = 0;
...
void data_callback(ma_device* pDevice, void* pOutput, const void* pInput, ma_uint32 frameCount) {
    ...
    if (CH1Enabled) {
        for (ma_uint64 iFrame = 0; iFrame < frameCount; iFrame += 1) {
            pFramesOutF32[iFrame*2] = current_clip[iFrame+start] * 60;
            pFramesOutF32[iFrame*2+1] = current_clip[iFrame+start] * 60;
        }
        for (int i = frameCount; i < clip_index;i++){
            current_clip[i-frameCount] = current_clip[i];
        }
        clip_index -= frameCount;
...
void APUInit() {
    ...
    apu_data = (uint8_t*) calloc(100000, sizeof(uint8_t));
}
...
void APUTick(int cycles) {
    if (CH1Enabled) {
        CH1Timer += cycles;
        if (CH1Timer > 0x7ff) {
            CH1Timer = CH1Period + (CH1Timer % 0x800);
            CH1_wave_index = (CH1_wave_index+1) % 8;
        }
    }
    if (CH2Enabled) {
        CH2Timer += cycles;
    ...
    if (appending_timer >= 83 && CH1Enabled) { // 2^22 / 48000
        appending_timer = 0;
